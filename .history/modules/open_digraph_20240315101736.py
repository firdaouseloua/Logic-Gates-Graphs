...
class OpenDigraph:  # for open directed graph
    ...
    def Dijikstra(self, src : int, direction= None) -> Dict[int, List[int]]:
        ...
        if direction == None:
            ...
            nodes = Depart.get_parents()
        else:
            raise ValueError("Direction can only be 1 -1 or None")
            
        nodes = [node[0] for node in nodes]
        
        for node in nodes:
            dist[node] = None
            prev[node] = None # at first the distance and last node is not defined
...
def min_distance(dictio):
    mini = None
    for node in dictio:
        if mini == None:
            mini = node
        else:
            if dict[node] < dict[mini]:
                mini = node
...
dict1 = {1: 0, 3:2, 4:1}
dict2 = {2:8, 7:3, 5: 2, 9:5}

print(min_distance(dict1))
print(min_distance(dict2))
